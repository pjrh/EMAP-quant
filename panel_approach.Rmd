...
IMD_fd_2015_2019 <- IMD_main_panel %>%
  ...
  arrange(year) %>%
  group_by(FeatureCode) %>%
  summarise(close_legal_aid_1mile_fd = 
              close_legal_aid_1mile[2] - close_legal_aid_1mile[1],
            close_legal_aid_4plusmile_fd = 
              close_legal_aid_4plusmile[2] - close_legal_aid_4plusmile[1])


LSOA_2011 %>%
  left_join(IMD_fd_2015_2019,
            by = c("lsoa11cd" = "FeatureCode")) %>%
  filter(close_legal_aid_1mile_fd != 0) %>%
  #filter(CTRY11NM == "England") %>%
  tm_shape() +
  tm_fill(col = "close_legal_aid_1mile_fd",
          style = "cat") +
  tm_shape(LA_shapes) +
  tm_borders(alpha = 0.2) +
  tm_shape(RoI_London) +
...
LSOA_2011 %>%
  left_join(IMD_fd_2015_2019,
            by = c("lsoa11cd" = "FeatureCode")) %>%
  filter(close_legal_aid_4plusmile_fd != 0) %>%
  #filter(CTRY11NM == "England") %>%
  tm_shape() +
  tm_fill(col = "close_legal_aid_4plusmile_fd",
          style = "cat") +
  tm_shape(LA_shapes) +
  tm_borders(alpha = 0.2) +
  tm_shape(RoI_London) +
