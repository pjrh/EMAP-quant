...
t1 <- lm(b_diff ~ close_legal_aid_near_2015 + close_legal_aid_far_2015 + close_jobcentre_near_2015 + close_jobcentre_far_2015 + close_gp_far_2015 + close_gp_near_2015,
   data = IMD_main_wide) %>%
   tbl_regression(intercept = TRUE,
                  estimate_fun = \(x) style_sigfig(x, digits = 3))

t2 <- lm(b_diff ~ RUC11_2019 + close_legal_aid_near_2015 + close_legal_aid_far_2015 + close_jobcentre_near_2015 + close_jobcentre_far_2015 + close_gp_far_2015 + close_gp_near_2015,
   data = IMD_main_wide) %>%
   tbl_regression(intercept = TRUE,
                  estimate_fun = \(x) style_sigfig(x, digits = 3))
...
tbl_merge(
   ...
   gt::text_transform(locations = gt::cells_body(columns = label),
                      \(x) case_when(x == "close_legal_aid_near_2015" ~ glue("Legal aid: within {near} miles"),
                                     x == "close_legal_aid_far_2015" ~ glue("Legal aid: Over {far} miles"),
                                     x == "close_jobcentre_near_2015" ~ glue("Jobcentre: within {near} miles"),
                                     x == "close_jobcentre_far_2015" ~ glue("Jobcentre: Over {far} miles"),
                                     x == "close_gp_near_2015" ~ glue("GP: within {near} miles"),
                                     x == "close_gp_far_2015" ~ glue("GP: Over {far} miles"),
                                     x == "RUC11_2019" ~ glue("ONS Rural/Urban classification"),
                                     TRUE ~ x)) %>%
  gt::gtsave(filename = "outputs/ols.html")
...
tbl_merge(
   ...
   as_gt() %>%
   gt::text_transform(locations = gt::cells_body(columns = label),
                      \(x) case_when(x == "close_legal_aid_near" ~ glue("Within {near} miles"),
                                     x == "close_legal_aid_far" ~ glue("Over {far} miles"),
                                     TRUE ~ x)) %>%
